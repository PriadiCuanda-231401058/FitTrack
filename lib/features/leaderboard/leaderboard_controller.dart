  ...
  Future<List<Map<String, dynamic>>> getWeeklyLeaderboard() async {
    try {
      ...
      for (var userDoc in usersSnapshot.docs) {
        ...
        final totalWorkouts =
            userData['progress']?['1 week']?['total_training'] ?? 0;
        ...
        leaderboard.add({'name': name, 'point': points, 'userId': userId});
      }
      ...
      return leaderboard
          .map((entry) => {'name': entry['name'], 'point': entry['point']})
          .toList();
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> getMonthlyLeaderboard() async {
    try {
      ...
      for (var userDoc in usersSnapshot.docs) {
        ...
        final totalWorkouts =
            userData['progress']?['1 month']?['total_training'] ?? 0;
